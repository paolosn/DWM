import { RULE_DWM_FRONTMATTER_KEY, type RuleMetadata } from "./RuleTypes.js";
 ...
 * encuentra su cierre: esa es la señal de una regla estructuralmente
 * inválida.
 */
...
const DWM_KEY_LINE = new RegExp(`^${RULE_DWM_FRONTMATTER_KEY}:\\s*$`);
...
export function serializeDwmBlock(metadata: RuleMetadata): string {
  const lines = [
...
export function upsertDwmBlock(frontmatter: string | undefined, metadata: RuleMetadata): string {
...
): Partial<RuleMetadata> | undefined {
  const lines = toLines(frontmatter);
  const range = findDwmBlockRange(lines);
...
/**
 * Extrae, de forma heurística y sin validarlo, un título legible de una
 * regla: primero busca `title:` en su frontmatter propio, y si no lo
 * encuentra, el primer encabezado `# ...` del cuerpo.
 */
export function extractRuleTitle(content: string): string | undefined {
