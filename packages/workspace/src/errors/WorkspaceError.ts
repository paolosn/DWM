 ...
 * (congelado). `WorkspaceError` reproduce la misma forma (`code`, `message`,
 * `origin`, `cause`, `recoverable`, `timestamp`, `toJSON()`) como clase
 * propia de `@dwm/workspace`, con su propio catálogo cerrado.
 */
export type WorkspaceErrorOrigin =
  "path" | "registry" | "configuration" | "scan" | "persistence" | "lifecycle";
...
export class WorkspaceError extends Error {
  ...
  static wrap(
    ...
    const message =
      options.message ??
      (err instanceof Error ? err.message : "Error desconocido en el espacio de trabajo");
